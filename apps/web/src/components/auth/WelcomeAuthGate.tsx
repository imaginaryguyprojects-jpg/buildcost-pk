...
export function WelcomeAuthGate({ onGuestAccess }: WelcomeAuthGateProps) {
  ...
  const { user, isAuthenticated, login, loginWithGoogle, signup, resendVerificationEmail, showToast } = useAuthStore();
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full py-2.5 px-4 rounded-xl bg-white hover:bg-slate-100 text-slate-900 border border-slate-200 dark:border-slate-700 font-bold text-xs shadow-sm transition-all flex items-center justify-center gap-2.5 cursor-pointer disabled:opacity-50"
                  >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">
                      {activeTab === "signin" ? "or sign in with email" : "or sign up with email"}
                    </span>
                    <div className="h-px bg-slate-200 dark:bg-slate-700 flex-1" />
                  </div>
                </div>
