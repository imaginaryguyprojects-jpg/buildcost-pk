  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Bill of Quantities (BOQ) Studio</h1>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-semibold shadow-md transition-all"
          >
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {items.map((row, idx) => (
                <tr key={row.id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="py-3 px-3 font-semibold text-slate-300">{row.category}</td>
                  <td className="py-3 px-3 text-slate-200">{row.description}</td>
                  <td className="py-3 px-3 text-center font-mono text-slate-400">{row.unit}</td>
        ...
        {/* Totals Section */}
        <div className="bg-slate-950 p-6 border-t border-slate-800 flex flex-col md:flex-row items-end justify-between gap-4">
          <div className="flex items-center gap-4 text-xs">
            <div>
              ...
              <input
                ...
                className="w-20 bg-slate-900 border border-slate-800 rounded-lg p-1.5 text-slate-200 text-xs"
              />
            </div>
            <div>
              <label className="text-slate-400 block mb-1">Contingency %</label>
              <input
                type="number"
                min="0"
                max="15"
          ...
          <div className="w-full md:w-72 space-y-1.5 text-xs text-right">
            <div className="flex justify-between text-slate-400">
              <span>Subtotal:</span>
              <span className="font-mono text-slate-200">{formatPKR(subtotal)}</span>
            </div>
            {discountPercent > 0 && (
            ...
            <div className="flex justify-between text-slate-400">
              ...
              <span className="font-mono text-slate-200">+{formatPKR(contingencyAmount)}</span>
            </div>
